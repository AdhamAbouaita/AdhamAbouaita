<div align="left">

# Computer Scientist | Software Engineer | Project Manager

I’m a Computer Scientist based in **Oakville, Ontario**. I lead a team of developers working on **[WMOJ](https://wmoj.ca)**, the official White Oaks Competitive Programming platform. I also write code to automate tutoring workflows for local high-schools. 

---

## 🚀 Featured Projects

### 🏆 [WOSS Competitive Programming Platform (WMOJ)](https://wmoj.ca)
*Project Lead & Lead Developer*

I lead a team of **7 developers** in building and maintaining the official competitive programming judge for White Oaks Secondary School. This project has been a significant exercise in full-stack engineering and technical leadership.
* **Infrastructure:** Architected a scalable microservices environment using **Docker** and **Kubernetes**.
* **DevOps:** Implemented robust CI/CD pipelines via **Jenkins** to ensure seamless deployment and code quality.
* **Leadership:** Managed the product roadmap, conducted code reviews, and coordinated the development efforts of a cross-functional team to deliver a reliable platform for the student body.

### 📚 [Tutoring Automation](https://github.com/Scire-apps/tutoring-automation)
*Developer*

Collaborated with a team of **3 developers** to engineer a solution for automating peer tutoring logistics. We focused on replacing manual data entry with automated scheduling workflows, significantly reducing the administrative burden for local high school programs and ensuring reliable data handling.

---

### 🧐 What else I'm up to

- 📱 **Side Quest:** Developing **[WorkingWeight](https://github.com/AdhamAbouaita/WorkingWeight)** to simplify progress tracking, because manual logging is for 2010.
- 🍔 **Magnum Opus:** I built **[FoodPredictor](https://github.com/AdhamAbouaita/FoodPredictor)** because I needed statistical analysis to determine if I should be excited for dinner.
- 🎮 **Retro Gaming:** Recreated the classic block-stacking experience with **[wetris](https://wetris.wmoj.ca)**.

---

### 🛠️ Tools I use to break (and fix) things

<div align="center">

  <a href="https://www.typescriptlang.org/" target="_blank"><img style="margin: 10px" src="https://profilinator.rishav.dev/skills-assets/typescript-original.svg" alt="TypeScript" height="50" /></a>
  <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript" target="_blank"><img style="margin: 10px" src="https://profilinator.rishav.dev/skills-assets/javascript-original.svg" alt="JavaScript" height="50" /></a>
  <a href="https://www.python.org/" target="_blank"><img style="margin: 10px" src="https://profilinator.rishav.dev/skills-assets/python-original.svg" alt="Python" height="50" /></a>
  <a href="https://www.java.com/" target="_blank"><img style="margin: 10px" src="https://profilinator.rishav.dev/skills-assets/java-original-wordmark.svg" alt="Java" height="50" /></a>
  <a href="https://isocpp.org/" target="_blank"><img style="margin: 10px" src="https://profilinator.rishav.dev/skills-assets/cplusplus-original.svg" alt="C++" height="50" /></a>
  <a href="https://en.wikipedia.org/wiki/C_(programming_language)" target="_blank"><img style="margin: 10px" src="https://profilinator.rishav.dev/skills-assets/c-original.svg" alt="C" height="50" /></a>

  <br />

  <a href="https://www.docker.com/" target="_blank"><img style="margin: 10px" src="https://profilinator.rishav.dev/skills-assets/docker-original-wordmark.svg" alt="Docker" height="50" /></a>
  <a href="https://kubernetes.io/" target="_blank"><img style="margin: 10px" src="https://profilinator.rishav.dev/skills-assets/kubernetes-icon.svg" alt="Kubernetes" height="50" /></a>
  <a href="https://aws.amazon.com/" target="_blank"><img style="margin: 10px" src="https://profilinator.rishav.dev/skills-assets/amazonwebservices-original-wordmark.svg" alt="AWS" height="50" /></a>
  <a href="https://www.jenkins.io/" target="_blank"><img style="margin: 10px" src="https://profilinator.rishav.dev/skills-assets/jenkins-icon.svg" alt="Jenkins" height="50" /></a>
  <a href="https://www.tensorflow.org/" target="_blank"><img style="margin: 10px" src="https://profilinator.rishav.dev/skills-assets/tensorflow-icon.svg" alt="TensorFlow" height="50" /></a>

  <br /> <br />

  <img src="https://img.shields.io/badge/Machine%20Learning-F7931E?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/Prompt%20Engineering-4285F4?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/CI%2FCD-2088FF?style=for-the-badge&logo=github-actions&logoColor=white" />
  <img src="https://img.shields.io/badge/Agile-0052CC?style=for-the-badge&logo=jira&logoColor=white" />

</div>

---


<br />



</div>
